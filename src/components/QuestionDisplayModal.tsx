...
export const QuestionDisplayModal: React.FC<QuestionDisplayModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-w-text-main/40 backdrop-blur-sm animate-fade-in overflow-y-auto">
      <div className="bg-w-bg-card border border-w-border w-full max-w-xl sm:max-w-2xl rounded-[22px] sm:rounded-[26px] shadow-[0_12px_36px_rgba(79,104,60,0.18)] overflow-hidden flex flex-col max-h-[calc(100dvh-1rem)] sm:max-h-[calc(100dvh-2rem)] my-auto wey-paper-card">
        {/* Header */}
        <div className="p-4 sm:p-5 bg-w-bg-main border-b border-w-border flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              {teamName && (
                <p className="text-xs font-[600] text-w-text-muted mt-0.5 flex items-center gap-1">
                  <span>Lượt trả lời:</span>
                  <span className="font-[800] text-w-text-main">{teamAvatar} {teamName}</span>
                </p>
              )}
            </div>
          </div>
          ...
          {timerEnabled && (
            <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-mono font-[800] border shadow-sm ${
              timeLeft <= 5
                ...
                : 'bg-w-accent-light text-w-text-main border-w-accent-border'
            }`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5">
          {mode === 'bank' && question ? (
            ...
            <div className="space-y-4">
              <div className="p-4 bg-w-bg-main/70 border border-w-border rounded-[20px]">
                <h3 className="text-base sm:text-lg font-[800] text-w-text-main leading-relaxed">
                  <MathChemRenderer text={question.content} />
                </h3>
                {question.imageUrl && (
                  <div className="mt-3 rounded-[16px] overflow-hidden border border-w-border bg-white flex items-center justify-center p-1 max-h-[220px]">
                    <img
              ...
              {question.type === 'mcq' && question.options && (
                <div className="space-y-2.5 pt-1">
                  <p className="text-xs font-[800] text-w-text-muted uppercase tracking-wider">
            ...
            <div className="py-4 space-y-4 text-center">
              ...
              <div>
                <h3 className="text-xl sm:text-2xl font-[800] text-w-text-main">CÂU HỎI SỐ #{questionNumber}</h3>
                <p className="text-xs font-[600] text-w-text-muted mt-1">
          ...
          {showResult && (
            <div className="space-y-3 pt-2 animate-fade-in">
              <div className={`p-4 rounded-[20px] text-center font-[800] text-sm sm:text-base border shadow-sm flex items-center justify-center gap-2 ${
                ...
                {isCorrect ? (
                  <>
                    <Sparkles className="w-5 h-5 text-w-primary shrink-0" />
              ...
              {mode === 'number' && (
                <div className="text-left space-y-1">
                  <label className="text-xs font-[800] text-w-text-muted">
        ...
        {/* Footer Action */}
        {showResult && (
          <div className="p-4 bg-w-bg-main border-t border-w-border flex items-center justify-end shrink-0">
            <button
              onClick={handleConfirm}
              className="w-full sm:w-auto px-6 py-2.5 bg-w-primary hover:bg-w-primary-hover text-white font-[800] text-xs sm:text-sm rounded-[15px] shadow-md transition cursor-pointer border border-w-primary-hover"
            >
              Xác Nhận & Tiếp Tục
            </button>
          </div>
