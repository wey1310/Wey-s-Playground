...
export const WerewolfGame: React.FC<WerewolfGameProps> = ({
  ...
  // 2. STATE MANAGEMENT
  const [skipQuestions, setSkipQuestions] = useState<boolean>(config.werewolfSkipQuestions ?? false);
  ...
  const handleProceedToQuestion = () => {
    setIsDawnModalOpen(false);
    if (skipQuestions) {
      // ⚡ Bỏ qua câu hỏi => Vào thẳng lượt bỏ phiếu treo cổ
  ...
  const checkAdvanceToNextNight = () => {
    // Check end condition:
    // 1. All wolves are eliminated / revealed
    ...
    // Ma sói kết thúc khi:
    // - Toàn bộ Ma Sói bị tiêu diệt hoặc bị vạch trần (Dân làng thắng!)
    // - Số Dân Làng sống <= Số Ma Sói sống (Sói thắng!)
  ...
  return (
    <div className="min-h-screen bg-[#070b14] flex flex-col justify-between text-slate-100 select-none">
      ...
      <WerewolfScoreboard
        ...
        onToggleSkipQuestions={() => setSkipQuestions(prev => !prev)}
