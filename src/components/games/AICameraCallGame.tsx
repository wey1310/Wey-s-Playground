...
export const AICameraCallGame: React.FC<GameProps> = ({ config, questions = [], onGameEnd, themeType = 'star' }) => {
  ...
  const handleLogic = (gesture: number) => {
    if (mode === 'rollcall') {
      ...
      } else if ([1, 2, 3].includes(gesture) && isAnswering) {
        // Answer logic
        const q = questions[currentQIndex];
        if (!q) return;
        const options = [...(q.incorrectAnswers || []), q.correctAnswer].sort();
        const correctIndex = options.indexOf(q.correctAnswer) + 1; // 1, 2, or 3
      ...
      <div className="absolute inset-0 z-20 pointer-events-none">
        ...
        <header className="p-6 flex justify-between items-start pointer-events-auto">
          ...
          <div className="flex flex-col items-end gap-3">
            <button
              onClick={() => onGameEnd([], [])}
              className="px-4 py-2 bg-red-500/80 hover:bg-red-500 rounded-xl font-bold flex items-center gap-2"
            >
                    ...
                    return options.map((opt, idx) => (
                      <div
                        ...
                        className={`p-6 rounded-2xl text-xl font-bold text-center border-4 transition-all ${
                          !isAnswering && opt === q.correctAnswer
