...
import { getRandomVillageCast, PERSONALITY_PROFILES } from './werewolfVillageData';
import { generateInitialNPCStatement } from './werewolfSimulationEngine';
...
export function initializeVillageNPCs(options: RoleEngineOptions = {}): NPCVillager[] {
  ...
  const templates = getRandomVillageCast(12);
  ...
  const npcs: NPCVillager[] = templates.map((tmpl, idx) => {
    ...
    const initialStmt = generateInitialNPCStatement(npcObj);
    npcObj.statement = initialStmt;
    npcObj.statementHistory = [initialStmt];
