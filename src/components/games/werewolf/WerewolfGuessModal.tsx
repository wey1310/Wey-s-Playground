...
export const WerewolfGuessModal: React.FC<WerewolfGuessModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-black/70 backdrop-blur-md select-none overflow-y-auto animate-fade-in">
      <motion.div
        initial={{ scale: 0.95, opacity: 0, y: 20 }}
        animate={{ scale: 1, opacity: 1, y: 0 }}
        className="w-full max-w-2xl bg-[#FFFDF5] border-2 border-[#DED5B8] rounded-3xl shadow-[0_25px_60px_rgba(0,0,0,0.35)] overflow-hidden flex flex-col max-h-[92vh]"
      >
        {/* Header */}
        ...
        <div className="p-5 sm:p-6 space-y-6 overflow-y-auto flex-1">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2.5 max-h-48 overflow-y-auto p-1.5 bg-[#F8F3E5] rounded-2xl border border-[#DED5B8] custom-scrollbar">
              {aliveNpcs.map((npc) => {
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-xl border-2 transition text-left flex items-center gap-2.5 cursor-pointer ${
                      isSelected
                        ...
                        : 'bg-white border-[#DED5B8] hover:border-amber-300'
                    }`}
                  >
          ...
          {selectedNpc && (
            <div className="p-3.5 bg-gradient-to-r from-amber-50 via-white to-amber-50 rounded-2xl border-2 border-amber-200 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600 mt-0.5">
                    Tính cách: <strong>{PERSONALITY_PROFILES[selectedNpc.personality]?.name}</strong> {PERSONALITY_PROFILES[selectedNpc.personality]?.badge} — {PERSONALITY_PROFILES[selectedNpc.personality]?.description}
                  </p>
                </div>
              </div>
                ...
                <button
                  ...
                  className={`p-4 rounded-2xl border-2 text-center transition flex flex-col items-center justify-center gap-2 cursor-pointer ${
                    guessValue === 'yes'
                      ...
                      : 'bg-white border-[#DED5B8] hover:bg-red-50/50 text-slate-700'
                  }`}
                >
                ...
                <button
                  ...
                  className={`p-4 rounded-2xl border-2 text-center transition flex flex-col items-center justify-center gap-2 cursor-pointer ${
                    guessValue === 'no'
                      ...
                      : 'bg-white border-[#DED5B8] hover:bg-emerald-50/50 text-slate-700'
                  }`}
                >
                  <span className="text-3xl">🌿</span>
                  <div>
                ...
                {roleOptions.map((opt) => (
                  <button
                    ...
                    className={`p-3 rounded-xl border-2 text-left transition flex items-center gap-2.5 cursor-pointer ${
                      guessValue === opt.role
                        ? 'bg-amber-50 border-amber-500 ring-2 ring-amber-400 shadow-sm'
                        : 'bg-white border-[#DED5B8] hover:bg-slate-50'
                    }`}
                  >
