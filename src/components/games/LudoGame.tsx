        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-1 bg-amber-300/80 p-1.5 rounded-xl border border-amber-600 text-xs font-bold text-amber-950">
            ...
            <button
              ...
              className={`px-2 py-0.5 rounded-lg transition ${diceCount === 1 ? 'bg-amber-700 text-white shadow' : 'hover:bg-amber-400 text-amber-950'}`}
            >
              1
            </button>
            <button
              onClick={() => setDiceCount(2)}
              className={`px-2 py-0.5 rounded-lg transition ${diceCount === 2 ? 'bg-amber-700 text-white shadow' : 'hover:bg-amber-400 text-amber-950'}`}
            >
              2
            </button>
          </div>
              ...
              {PATH_MAP.map((pos, idx) => {
                ...
                let startBg =
                  idx === 0
                    ...
                    : 'bg-white/80 border-amber-300 text-stone-800';
              ...
              {Object.entries(HOMES).map(([teamKey, homeCells]) => {
                ...
                return homeCells.map((pos, idx) => (
                  <div
                    ...
                    style={{ gridRowStart: pos.r, gridColumnStart: pos.c }}
                    className={`rounded-lg border border-amber-800/60 ${bg} flex items-center justify-center text-[9px] font-extrabold shadow-inner`}
                  >
                    {idx + 1}
                  </div>
                ));
