...
export const ImportQuestionsModal: React.FC<ImportQuestionsModalProps> = ({
  ...
  return (
    <div id="import-questions-modal" className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-3 sm:p-4 animate-fade-in overflow-y-auto">
      <div className="bg-white rounded-3xl w-full max-w-4xl overflow-hidden shadow-2xl flex flex-col max-h-[92vh] my-auto">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold text-slate-800">Nạp Câu Hỏi Tự Động Với AI</h2>
              <p className="text-xs text-slate-500">Hỗ trợ trắc nghiệm (MCQ A-D), JSON chuẩn, đúng/sai (TF) & trả lời ngắn</p>
            </div>
          </div>
          <button 
            id="close-import-modal-btn"
            onClick={onClose} 
            className="p-2 rounded-xl text-slate-400 hover:bg-slate-200 hover:text-slate-700 transition cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
          ...
          <div className="flex gap-2 shrink-0">
            <button
              ...
              className={`flex-1 py-2.5 px-4 rounded-xl flex justify-center items-center gap-2 font-bold text-xs sm:text-sm transition cursor-pointer ${
                mode === 'file' ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-2.5 px-4 rounded-xl flex justify-center items-center gap-2 font-bold text-xs sm:text-sm transition cursor-pointer ${
                mode === 'text' ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
              }`}
            >
          ...
          {(error || (diagnostics && diagnostics.status === 'invalid') || (diagnostics && diagnostics.issues && diagnostics.issues.length > 0)) && (
            <div className="p-4 bg-rose-50 border border-rose-200 text-rose-800 rounded-2xl text-xs space-y-2 shadow-sm animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 sm:gap-6">
            <div className="md:col-span-3 space-y-3">
              {mode === 'file' ? (
                <div 
                  id="import-dropzone"
                  className="border-2 border-dashed border-slate-200 rounded-2xl p-6 sm:p-8 flex flex-col items-center justify-center text-center bg-slate-50 hover:bg-indigo-50 hover:border-indigo-300 transition cursor-pointer min-h-[280px]"
                  onClick={() => fileInputRef.current?.click()}
                >
                  <div className="w-14 h-14 bg-white rounded-full flex items-center justify-center text-indigo-500 mb-3 shadow-sm border border-slate-100">
                  ...
                  <p className="text-xs text-slate-500 max-w-[280px] mb-4">
                    AI sẽ tự động đọc slide PPTX, trích xuất trắc nghiệm, đúng/sai, tự luận & nhận diện đáp án chuẩn
                  </p>
                  <button className="px-5 py-2 bg-indigo-600 text-white text-xs font-bold rounded-xl hover:bg-indigo-700 transition cursor-pointer">
                ...
                <div className="flex flex-col h-full min-h-[280px]">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-xs font-bold text-slate-700">Nội dung câu hỏi hoặc JSON:</span>
                    <button
                      type="button"
                  ...
                  <textarea
                    ...
                    className="flex-1 w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 text-xs sm:text-sm font-mono text-slate-700 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 resize-none min-h-[180px]"
                  />
                  <div className="mt-3 flex justify-end">
                    <button
                      ...
                      className="px-5 py-2.5 bg-indigo-600 text-white text-xs sm:text-sm font-bold rounded-xl hover:bg-indigo-700 transition disabled:opacity-50 flex items-center gap-2 cursor-pointer shadow-sm"
                    >
            ...
            {/* Guide Sidebar */}
            <div className="md:col-span-2 bg-amber-50/80 border border-amber-200 rounded-2xl p-4 flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between mb-2">
                  <h4 className="flex items-center gap-1.5 font-bold text-amber-900 text-xs sm:text-sm">
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 bg-white border border-amber-300 text-amber-800 rounded-lg text-[10.5px] font-bold hover:bg-amber-100 flex items-center gap-1 cursor-pointer"
                  >
                ...
                <div className="flex flex-wrap gap-1 mb-2.5">
                  <button
                    ...
                    className={`text-[10px] px-2.5 py-1 rounded-md font-bold transition cursor-pointer flex items-center gap-1 ${
                      selectedGuideTab === 'mcq' ? 'bg-amber-800 text-white' : 'bg-white text-amber-800 hover:bg-amber-100'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`text-[10px] px-2.5 py-1 rounded-md font-bold transition cursor-pointer flex items-center gap-1 ${
                      selectedGuideTab === 'json' ? 'bg-amber-800 text-white' : 'bg-white text-amber-800 hover:bg-amber-100'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`text-[10px] px-2.5 py-1 rounded-md font-bold transition cursor-pointer flex items-center gap-1 ${
                      selectedGuideTab === 'all' ? 'bg-amber-800 text-white' : 'bg-white text-amber-800 hover:bg-amber-100'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`text-[10px] px-2.5 py-1 rounded-md font-bold transition cursor-pointer flex items-center gap-1 ${
                      selectedGuideTab === 'tf' ? 'bg-amber-800 text-white' : 'bg-white text-amber-800 hover:bg-amber-100'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`text-[10px] px-2.5 py-1 rounded-md font-bold transition cursor-pointer flex items-center gap-1 ${
                      selectedGuideTab === 'text' ? 'bg-amber-800 text-white' : 'bg-white text-amber-800 hover:bg-amber-100'
                    }`}
                  >
                ...
                <div className="bg-slate-900 border border-slate-800 text-slate-100 rounded-xl p-2.5 text-[11px] font-mono space-y-1 shadow-inner max-h-48 overflow-y-auto whitespace-pre-wrap">
                  {SAMPLE_TEMPLATES[selectedGuideTab]}
                </div>
              </div>

              <div className="mt-3 pt-2.5 border-t border-amber-200 space-y-1.5 text-[10.5px] text-amber-900">
                <div className="flex items-start gap-1.5">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 mt-0.5 shrink-0" />
                  <span>Dùng <code className="bg-white/80 px-1 rounded">Câu X:</code> bắt đầu câu hoặc mảng <code className="bg-white/80 px-1 rounded">[ ... ]</code> JSON.</span>
                </div>
                <div className="flex items-start gap-1.5">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 mt-0.5 shrink-0" />
                  <span>Ghi <code className="bg-white/80 px-1 rounded">Đáp án: C</code> hoặc gắn hoa thị <code className="bg-white/80 px-1 rounded">*C.</code></span>
                </div>
                <div className="pt-1 flex justify-end">
                  <button
                    type="button"
                    onClick={() => insertSampleText(selectedGuideTab)}
                    className="px-2.5 py-1 bg-amber-700 hover:bg-amber-800 text-white rounded-lg text-[10.5px] font-bold flex items-center gap-1 cursor-pointer shadow-xs"
                  >
