...
export function sanitizeLogMessage(input: any): string {
  ...
  // Mask Google API key patterns: AIzaSy... (39 chars) or AQ....
  str = str.replace(/AIza[0-9A-Za-z-_]{35}/g, (match) => `${match.substring(0, 6)}...[REDACTED]`);
  str = str.replace(/AQ\.[0-9A-Za-z-_]{30,}/g, (match) => `${match.substring(0, 6)}...[REDACTED]`);
...
export function getMaskedKeyHint(apiKey?: string): {
  ...
  const isStandard = (clean.startsWith('AIza') && len === 39) || (clean.startsWith('AQ.') && len >= 40);
...
export async function logApiValidationAttempt(params: {
  ...
  try {
    ...
    // 1. Structured Console Output (An toàn, hữu ích cho debugging trong Cloud Run / Container / Vercel logs)
    if (params.success) {
      console.log(
    ...
    // Fail-safe fallback so logging never breaks request execution
    console.error("⚠️ Validation logging exception:", fatalLogErr);
    return {
      id: `err_${Date.now()}`,
